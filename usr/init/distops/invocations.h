...
//{{{1 Cap zeroing
static inline errval_t
invoke_monitor_nullify_domcap(capaddr_t root_cap, capaddr_t root_level, capaddr_t cap, int level)
{
    return cap_invoke5(cap_kernel, KernelCmd_Nullify_domcap, root_cap, root_level, cap, level).error;
}
...
errval_t monitor_nullify_domcap(struct capref croot, capaddr_t cap, int level);
