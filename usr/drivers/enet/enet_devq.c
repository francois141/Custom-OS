                                ...
                                uint64_t* flags)
{
    ...
    *flags = buf->flags;
    ...
    /* No invalidate needed here, we do that
     * on the enqeue side
     */
                                ...
                                genoffset_t valid_length, uint64_t flags)
{
    ...
    /*
    * Write back so the NIC sees the enqueued data
    * Also invalidate so we don't read stale values
    * when we dequeue next time. Also make sure the
    * data is written back before we pass the descriptor
    * to the NIC.
    */
    ...
    /*
    * Write back so the NIC sees the updated descriptor
    * Also invalidate so we don't read stale values
    * when we dequeue next time.
    */
    cpu_dcache_wbinv_range((lvaddr_t) &q->ring[q->tail], sizeof(enet_bufdesc_t));
                                ...
                                genoffset_t valid_length, uint64_t flags)
{
    ...
    /*
    * Invalidate so we don't read stale values
    * when we dequeue next time. We have to also
    * writeback as invalidate alone is not permitted from
    * userspace.
    * Also make sure the invalidation is done before we pass
