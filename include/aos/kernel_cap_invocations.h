...
/**
 * \brief Create a PhysAddr cap ab initio, by invoking the kernel cap.
 *
 * \param dest   Location to place new Frame cap
 * \param base   Base address of the region
 * \param bytes  Size of region to create
 ...
 */
static inline errval_t
physaddr_forge(struct capref dest, genpaddr_t base, gensize_t bytes,
                        coreid_t coreid) {
    struct capability frame_cap = {
        .type = ObjType_PhysAddr,
        .rights = CAPRIGHTS_READ_WRITE,
        .u.frame = {
            .base  = base,
            .bytes = bytes
