...
{
    size_t frame_size = 0;
    errval_t err = frame_create(frame_slot, minbytes, &frame_size);
    if (err_is_fail(err)) {
        DEBUG_ERR(err, "slab refill frame create\n");
        return err;
    }

    void* buf = NULL;
    err = paging_map_frame(get_current_paging_state(), &buf, frame_size, frame_slot);
    if (err_is_fail(err)) {
        DEBUG_ERR(err, "slab refill map frame");
        return err;
    }
