...
static errval_t fs_mkdir_fail(const char *path){ (void)path; FAIL_FN(); }
static errval_t fs_rmdir_fail(const char *path){ (void)path; FAIL_FN(); }
static errval_t fs_rm_fail(const char *path){ (void)path; FAIL_FN(); }
static errval_t fs_opendir_fail(const char *path, fs_dirhandle_t *h){ (void)path; (void)h;FAIL_FN(); }
static errval_t fs_readdir_fail(fs_dirhandle_t h, char **name) { (void)h; (void)name; FAIL_FN(); }
static errval_t fs_closedir_fail(fs_dirhandle_t h) { (void)h;FAIL_FN(); }
static errval_t fs_fstat_fail(fs_dirhandle_t h, struct fs_fileinfo *b) {(void)h;(void)b; FAIL_FN(); }
