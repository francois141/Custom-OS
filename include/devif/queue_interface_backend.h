...
// Creation of queues is device specific
  ...
  *        On a notification, the device can dequeue descritpors
...
typedef errval_t (*devq_control_t)(struct devq *q,
  ...
  * @param valid_data   Offset into the buffer where the valid data of the
  *                     buffer starts
...
typedef errval_t (*devq_enqueue_t)(struct devq *q, regionid_t region_id,
  ...
  * @param valid_offset Return pointer to the offset into the buffer
...
/**
 * @brief Get the number of bytes that must be reserved at the beginning of each packet buffer
 *
 * @param q The device queue to query
 * @return The number of bytes.
 * @note The network stack should reserve at least the returned number of bytes
 *       at the beginning of each network packet buffer and should reflect this
 *       property when invoking `devq_enqueue()` by passing a value of `valid_data`
 *       that is equal to or greater than the value returned by this function.
 */
typedef size_t (*devq_get_num_reserved_bytes_t)(struct devq *q);
...
struct devq_func_pointer {
    ...
    devq_get_num_reserved_bytes_t get_reserved_size;
};
