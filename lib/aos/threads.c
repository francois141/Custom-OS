...
static struct slab_allocator except_stack_slabs;
...
{
    const size_t refill_size = PAGING_PAGE_ALIGN(4 * slabs->blocksize);
    void* bufp = NULL;
    errval_t err = paging_alloc(get_current_paging_state(), &bufp, refill_size, BASE_PAGE_SIZE);
    if (err_is_fail(err)) {
        return err;
    }
    slab_grow(slabs, bufp, refill_size);
    return SYS_ERR_OK;
}

static errval_t refill_except_stack_slabs(struct slab_allocator *slabs)
{
    const size_t refill_size = PAGING_PAGE_ALIGN(4 * slabs->blocksize);
    ...
    errval_t err = frame_alloc(&cap, refill_size, NULL);
    if (err_is_fail(err)) {
        DEBUG_ERR(err, "refill_except_stack_slabs frame_alloc");
        return err;
    }
    ...
    if (err_is_fail(err)) {
        DEBUG_ERR(err, "exception stack frame map");
        return err;
    }
...
static void free_thread(struct thread *thread)
{
    ...
    slab_free(&except_stack_slabs, thread->default_except_stack);
                                        ...
                                        size_t stacksize)
{
    ...
    void *except_stack = slab_alloc(&except_stack_slabs);
    release_spinlock(&thread_slabs_spinlock);
    // thread_mutex_unlock(&thread_slabs_mutex);
    if (space == NULL) {
        free(stack);
    ...
    newthread->default_except_stack = except_stack;
    newthread->default_except_stack_size = PAGING_EXCEPT_STACK_SIZE;
    ...
    // {
    //     newthread->default_except_stack = NULL;
    //     newthread->default_except_stack_size = 0;

    //     errval_t err = frame_alloc(&newthread->default_except_stack_cap, PAGING_EXCEPT_STACK_SIZE, NULL);
    //     if (err_is_fail(err)) {
    //         DEBUG_ERR(err, "exception stack frame alloc");
    //         free_thread(newthread);
    //         free(stack);
    //         return NULL;
    //     }
    ...
    //     err = paging_map_frame(
    //         get_current_paging_state(), 
    //         &newthread->default_except_stack, 
    //         PAGING_EXCEPT_STACK_SIZE, 
    //         newthread->default_except_stack_cap
    //     );
    //     if (err_is_fail(err)) {
    //         DEBUG_ERR(err, "exception stack frame map");
    //         free_thread(newthread);
...
//int bootstrap_thread(struct spawn_domain_params *params)
{
    ...
    slab_init(&except_stack_slabs, PAGING_EXCEPT_STACK_SIZE, refill_except_stack_slabs);
...
{
    return thread_target_set_exception_handler(thread_self(), newhandler, oldhandler, new_stack_base, new_stack_top, old_stack_base, old_stack_top);
}

errval_t thread_target_set_exception_handler(struct thread* thread, 
...
{
    if (oldhandler != NULL) {
        *oldhandler = thread->exception_handler;
    }

    if (old_stack_base != NULL) {
        *old_stack_base = thread->exception_stack;
    }

    if (old_stack_top != NULL) {
        *old_stack_top = thread->exception_stack_top;
    }

    thread->exception_handler = newhandler;

    if (new_stack_base != NULL && new_stack_top != NULL) {
        thread->exception_stack = new_stack_base;
        thread->exception_stack_top = new_stack_top;
    }
