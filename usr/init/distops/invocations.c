 ...
 * \brief Invoke the kernel cap to set the type of #cap to null
 */
errval_t monitor_nullify_domcap(struct capref croot, capaddr_t caddr, int level)
{
    capaddr_t root_caddr = get_cap_addr(croot);
    uint8_t root_level = get_cap_level(croot);
    return invoke_monitor_nullify_domcap(root_caddr, root_level, caddr, level);
}
                                   ...
                                   gensize_t count)
{
    ...
    capaddr_t dest_root = get_croot_addr(dest_start);
    // Address of the cap to the destination CNode
    capaddr_t dest_addr = get_cnode_addr(dest_start);
    // Depth/Level of destination cnode
    enum cnode_type dest_level = get_cnode_level(dest_start);
    // Address of source cspace
    capaddr_t src_root = get_croot_addr(src);
    // Address of source capability
    capaddr_t src_addr = get_cap_addr(src);

    /*return invoke_monitor_remote_cap_retype(rootcap_addr, rootcap_level, src, offset,
      newtype, objsize, count, to, slot, level);*/
    return invoke_monitor_remote_cap_retype(src_root, src_addr, offset, newtype, objsize, count, dest_root, dest_addr,dest_start.slot ,dest_level);
}
...
errval_t monitor_domcap_retype_remote_cap(struct domcapref dest_start, struct domcapref src, gensize_t offset, enum objtype newtype, gensize_t objsize, gensize_t count, capaddr_t slot) {
    return invoke_monitor_remote_cap_retype(get_cap_addr(src.croot), src.cptr, offset, newtype, objsize, count, get_cap_addr(dest_start.croot), dest_start.cptr ,slot ,dest_start.level);
}
