...
errval_t slot_alloc_root(struct capref *ret)
{
    ...
    // If we got to 2 or below here somehow it's game over
    // unless we are in a refill
#ifndef NDEBUG
    rootcn_free = single_slot_alloc_freecount(&state->rootca);
    assert(rootcn_free > 2 || state->rootca.refilling);
#endif
...
errval_t root_slot_allocator_refill(cn_ram_alloc_func_t myalloc, void *allocst)
{
    ...
    // Resize rootcn
    err = cnode_replace_own_l1(newroot_cap, root_ram);
    if (err_is_fail(err)) {
