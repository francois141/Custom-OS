...
static void mmap_find_memory(struct multiboot_tag_efi_mmap *mmap)
{
    ...
    size_t ndesc = mmap->size / mmap->descr_size;

#ifdef PRINT_MMAP
    for (size_t i = 0; i < ndesc; i++) {
        efi_memory_descriptor *desc = (efi_memory_descriptor *)(mmap->efi_mmap + mmap->descr_size * i);
        printf("mmap raw:   [%012lx..%012lx] type: %d]\n",
        desc->PhysicalStart, desc->PhysicalStart + desc->NumberOfPages * BASE_PAGE_SIZE - 1, desc->Type);
    }
#endif

    // as it seems the memory map we get may not be sorted, so we sort it
    // performing a simple selection sort to sort the memory map in ascending order
    for (size_t i = 0; i < ndesc; i++) {
        efi_memory_descriptor *desc = (efi_memory_descriptor *)(mmap->efi_mmap + mmap->descr_size * i);

        size_t min_idx = i;
        uint64_t min_addr = desc->PhysicalStart;
        for (size_t j = i; j < ndesc; j++) {
            efi_memory_descriptor *other = (efi_memory_descriptor *)(mmap->efi_mmap + mmap->descr_size * j);
            if (other->PhysicalStart < min_addr) {
                min_addr = other->PhysicalStart;
                min_idx = j;
            }
        }
        if (min_idx != i) {
            // do the swapping!
            efi_memory_descriptor *other = (efi_memory_descriptor *)(mmap->efi_mmap + mmap->descr_size * min_idx);
            efi_memory_descriptor tmp = *desc;
            *desc = *other;
            *other = tmp;
        }
    }
    ...
    for (size_t i = 0; i < ndesc; i++) {
        efi_memory_descriptor *desc = (efi_memory_descriptor *)(mmap->efi_mmap + mmap->descr_size * i);
        printf("mmap sorted: [%012lx..%012lx] type: %d]\n",
        desc->PhysicalStart, desc->PhysicalStart + desc->NumberOfPages * BASE_PAGE_SIZE - 1, desc->Type);
    }
#endif
    ...
    for (size_t i = 0; i < mmap->size; i += mmap->descr_size) {
        ...
        if (desc->Type == EfiConventionalMemory && desc->NumberOfPages > ARMV8_CORE_DATA_PAGES) {
            assert(ROUND_UP(desc->PhysicalStart, BASE_PAGE_SIZE) == desc->PhysicalStart);
