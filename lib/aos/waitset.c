    ...
    do {
        ...
        switch (chan->chantype) {
            ...
            case CHANTYPE_UMP_IN: {
                struct ump_chan* ump_chan = (struct ump_chan*)chan->chan_data;
                chan_ready = ump_chan_can_recv(ump_chan);
                break;
            }
            case CHANTYPE_UMP_OUT: {
                struct ump_chan* ump_chan = (struct ump_chan*)chan->chan_data;
                chan_ready = ump_chan_can_send(ump_chan);
                break;
            }
            default:
